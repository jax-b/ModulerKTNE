...
SideControl::setSerialNumber(char serialnumber[8]) {
    ...
    if (ioctl(ModControl::file_i2c, I2C_SLAVE, SideControl::RIGHT_PANEL)) {
...
SideControl::clearSerialNumber() {
    ...
    if (write(ModControl::file_i2c, {0x20}, 1) != 1) {
        printf("Failed to write to the i2c bus.\n");
        return -1;
    }
    return 1;
...
SideControl::setIndicator(uint8_t panel, bool lit, char indicator[3]) {\
    ...
    if (ioctl(ModControl::file_i2c, I2C_SLAVE, panel)) {
        printf("Failed to acquire bus access and/or talk to slave.\n");
        return -1;
    }
...
SideControl::clearAllIndicator(uint8_t panel) {
    ...
    if (write(ModControl::file_i2c, {0x21}, 1) != 1) {
        printf("Failed to write to the i2c bus.\n");
        return -1;
    }
    return 1;
...
SideControl::setSideArt(uint8_t panel, uint8_t art]) {
    ...
    if (write(ModControl::file_i2c, {0x12, art}, 2) != 2) {
        printf("Failed to write to the i2c bus.\n");
        return -1;
    }
    return 1;
...
SideControl::clearAllSideArt(uint8_t panel) {
    ...
    if (write(ModControl::file_i2c, {0x22}, 1) != 1) {
        printf("Failed to write to the i2c bus.\n");
        return -1;
    }
    return 1;
